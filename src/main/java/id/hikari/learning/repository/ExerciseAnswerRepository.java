...
public interface ExerciseAnswerRepository extends JpaRepository<ExerciseAnswer, Long> {
    ...
    @Query(value = "select new id.hikari.learning.payload.ListExerciseResponse(e.exerciseName,ea.generateId,e.description,COUNT(ea),ea.createdBy,ea.creationDate) from ExerciseAnswer ea join Exercise e on ea.exerciseId=e.id group by ea.generateId order by ea.creationDate desc")
